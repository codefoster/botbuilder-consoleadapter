...
export class ConsoleAdapter extends BotAdapter {
     ...
     *
     * - A 'message' activity will be created containing the users input text.
    ...
    public listen(logic: (context: TurnContext) => Promise<void>): () => void {
        const rl: readline.ReadLine = readline.createInterface({ input: process.stdin, output: process.stdout, terminal: false });
